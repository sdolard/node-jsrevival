...
JsRevivalApp = (function() {
	...
	function JsRevivalApp (){
		...
		this.getProcessParams(function(err){
			this.run();
		}.bind(this));
	}
	...
	*/
	JsRevivalApp.displayHelp = function() {
		...
		console.log(util.format('jsrevival %s: a JSLint cli.', JsRevivalApp.getPackage().version));
		console.log('Options:');
	...
	JsRevivalApp.prototype = {
		...
		getProcessParams: function(cb) {
			...
			while ((opt = optParser.getopt()) !== undefined && !opt.error) {
				...
				this.jslint_options += "predef: ['" + opt.optarg.split(',').map(mapTrim).join("','") + "']";
				break;
