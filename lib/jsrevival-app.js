...
JsRevivalApp.displayHelp = function() {
    console.log('jsrevival [-j jslint_file] [-o jslint_options_file] [-s] [–m] [–v] [-R] [–q] [-p prefef] [-r reporterName] [-e] [–h] files directories ... ');
    ...
    console.log('  e: read reporter config from JSREVIVAL_REPORTER user environment');
...
JsRevivalApp.prototype = {
    ...
    getProcessParams: function() {
        ...
        optParser = new getopt.BasicParser(':heRvmqsj:p:o:r:', process.argv);
        while ((opt = optParser.getopt()) !== undefined && !opt.error) {
            ...
            case 'r': // reporter
                this.toLint.shift();
                this.toLint.shift();                
                this.reporterName = opt.optarg;
                break;
                
            case 'e': // reporter
                this.toLint.shift();
                this.reporterName = process.env.JSREVIVAL_REPORTER; 
                this._log('Reading reporter from user environment: ', this.reporterName);
                break;
